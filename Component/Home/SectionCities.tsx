...
const SectionCities = () => {
  return (
    <Grid container p={"2rem"} mt={"2rem"} bgcolor={"rgb(255, 255, 255)"} dir={"rtl"} borderTop={"1px solid #EDEFF0"}   sx={{ width: { xs: "12", lg: "11" }, display: { sm: "flex", xs: "none" } }} display={"flex"} justifyContent={"space-around"}>
       ...
       {
        DataSectionCities.map((item,index) =>(
            <Grid md={1} pt={2} flexBasis={"16.6667%"} sx={{cursor:"pointer" , maxWidth:"16.6667%" }} key={index} ><Typography variant='body2'>{item.name}</Typography></Grid>
        ))
       }
